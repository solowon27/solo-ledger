...
const navItems = [
  { name: 'Rate Calculator', href: '/', icon: Calculator },
    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200/80 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          ...
          {/* Desktop Mega-Menu for Benchmarks */}
          <div 
            className="relative"
            onMouseEnter={() => setBenchmarksOpen(true)}
            onMouseLeave={() => setBenchmarksOpen(false)}
          >
            <button className="group flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-bold text-slate-600 transition-all hover:bg-slate-50 hover:text-slate-900">
              <BriefcaseBusiness className="h-4 w-4 text-slate-400 group-hover:text-slate-600" />
              Benchmarks
              <ChevronDown className={`h-4 w-4 text-slate-400 transition-transform ${benchmarksOpen ? 'rotate-180' : ''}`} />
            </button>

            {benchmarksOpen && (
              <div className="absolute right-0 top-full mt-1 w-[480px] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(ROLES_DATA).map(([slug, data]) => (
                    <Link
                      ...
                      className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-700 transition-colors"
                    >
                      <div className="h-1.5 w-1.5 rounded-full bg-slate-300" />
                      <span className="truncate">{data.shortTitle}</span>
                    </Link>
                  ))}
                </div>
              </div>
      ...
      {mobileOpen && (
        <div className="absolute left-0 top-16 h-[calc(100vh-4rem)] w-full border-t border-slate-200 bg-white md:hidden overflow-y-auto pb-20">
          <nav className="flex flex-col gap-1 p-4">
            ...
            {/* Mobile Benchmarks List */}
            <div className="mt-4 mb-2 px-3 text-[10px] font-black uppercase tracking-wider text-slate-400">
              Industry Benchmarks
            </div>
            <div className="flex flex-col gap-1">
              {Object.entries(ROLES_DATA).map(([slug, data]) => (
                <Link
                  ...
                  href={`/rate/${slug}`}
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                >
